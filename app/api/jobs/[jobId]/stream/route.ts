  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      if (job) {
        ...
        if (job.stato === 'IN_ESECUZIONE' && fase && fase !== 'ricerca') {
          // Informa il client della fase corrente raggiunta finora
          enqueue({ jobId, fase, progresso, messaggio: `Ripresa da fase: ${fase}` })
        }
      }
